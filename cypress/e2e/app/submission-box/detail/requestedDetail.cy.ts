...
describe('Requested Dashboard Details Page Tests', () => {
    ...
    it('should display a submission box with no information inputted other than title and no video', () => {
        const title = 'Test Requested'
        cy.task('getUserId', email).then((userId) => {
            cy.task('createRequestSubmissionForUser', { userId, title })
        })
        cy.reload()
        cy.visit('/dashboard')
        runWithRetry(() => {
        ...
        cy.get('[data-cy="noSubmission"]', { timeout: TIMEOUT.EXTRA_LONG }).should('be.visible').and(
            'contain',
